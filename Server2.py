    ...
    def listenToClient(self, client, addr):
        ...
        while True:
            ...
            if (str_file.find(na[i]) != -1 and str_file.find(pas[i])):  # if you can find username
                ...
                arr6 = "Successfuly Authenticated!".encode()
                client.send(arr6)
                ...
                le = (str(len(questions)) + '\n').encode()
                client.send(le)
                #client.send('Question\n'.encode())
                score = 0
                for number in range(0, len(questions)):

                    var = (str(questions[number])).encode()
                    client.send(var)
                ...
                if timestamp > 30:
                    ...
                    client.close()
                    # display total score
                scr = na, su, "score", str(score), "bonus", str(10 / (timestamp + 1)), "total Score:", str(
                    score + 10 / (timestamp + 1))
                print(scr)
